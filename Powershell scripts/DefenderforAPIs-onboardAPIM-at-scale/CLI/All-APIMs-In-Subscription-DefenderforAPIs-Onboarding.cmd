...
# Query using Azure Resource Graph to get APIM services with their resource group across all subscriptions
queryResult=$(az graph query -q "Resources | where type =~ 'Microsoft.ApiManagement/service' | where subscriptionId == '$subscriptionId' | project id, name, resourceGroup, subscriptionId" -o json)

# Parse the JSON and iterate over the APIM services
echo "$queryResult" | jq -c '.data[]' | while read service; do
    ...
    # Get the API collections for each APIM service
